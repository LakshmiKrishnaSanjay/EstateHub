...
const ViewMore = () => {
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this property?")) {
      try {
        ...
        navigate("/agent/viewproperties"); // Redirect to list
      } catch (err) {
  ...
  if (error) return <div>Error loading property details: {error.message}</div>;
  ...
  return (
    <div>
      ...
      <div className="container mx-auto p-6">
        ...
        <p className="text-lg text-gray-700 mb-6">{data?.property?.description}</p>
    ...
    {data?.property?.propertyFor === "buy" ? "Sale" : "Rent"}
            ...
            <p className="text-lg mb-2">
              <strong>Price:</strong> {data?.property?.price.toLocaleString()}
            </p>
            {data?.property?.bedrooms != null && (
              <p className="text-lg mb-2">
                <strong>Bedrooms:</strong> {data?.property?.bedrooms}
              </p>
            )}
            {data?.property?.bathrooms != null && (
              <p className="text-lg mb-2">
                <strong>Bathrooms:</strong> {data?.property?.bathrooms}
              </p>
            )}
            {data?.property?.kitchens != null && (
              <p className="text-lg mb-2">
                <strong>Kitchen:</strong> {data?.property?.kitchens}
              </p>
            )}
<p className="text-lg mb-2">
  <strong>Available:</strong> {data?.property?.isAvailable ? "Yes" : "No"}
            ...
            <p className="text-lg mb-2">
              <strong>Area:</strong> {data?.property?.area} sq. ft.
            </p>
            <p className="text-lg mb-2">
            ...
            <p className="text-lg mb-2">
              <strong>Average Rating:</strong> {data?.property?.averageRating} ⭐
            </p>)}
